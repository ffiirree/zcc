...
#define __IN_SCOPE__(localEnv, preEnv, _name)                                                              \
    do {                                                                                                   \
        Env *old = preEnv;                                                                                 \
...
#define __OUT_SCOPE__(localEnv)                                                                            \
    do {                                                                                                   \
        localEnv = localEnv->pre();                                                                        \
        gen_quad(".outscope");                                                                             \
    } while (0)
 ...
 */
#define __IN_FUNC_DEF__(RET_TYPE)                                                                          \
    do {                                                                                                   \
        isFunction         = true;                                                                         \
        currentFuncRetType = RET_TYPE;                                                                     \
        noReturnValue      = true;                                                                         \
        localVarsSize_     = 0;                                                                            \
        paramsSize_        = 0;                                                                            \
        maxCallSize_       = 0;                                                                            \
    } while (0)
#define __OUT_FUNC_DEF__()                                                                                 \
    do {                                                                                                   \
        isFunction         = false;                                                                        \
        currentFuncRetType = nullptr;                                                                      \
    } while (0)

/**
 * \ param offset address
 */
#define __BEGIN_PARAMS__()                                                                                 \
    do {                                                                                                   \
        isParams    = true;                                                                                \
        paramsSize_ = 8;                                                                                   \
    } while (0)
#define __END_PARAMS__()                                                                                   \
    do {                                                                                                   \
        isParams = false;                                                                                  \
    } while (0)
...
public:
    StrCard()
        : _str(),
          _label()
    {}
    StrCard(const std::string& _s, const std::string& _l)
        : _str(_s),
          _label(_l)
    {}
    StrCard(const StrCard& bl)
        : _str(bl._str),
          _label(bl._label)
    {}
    StrCard operator=(const StrCard& bl)
    {
        _str   = bl._str;
        _label = bl._label;
        return *this;
...
public:
    ...
    Parser(TokenSequence& ts)
        : ts_(ts),
          quadStk_()
    {}
    Parser(TokenSequence& ts, const std::string& _ofn)
        : ts_(ts),
          _of_name(_ofn + ".q"),
          quadStk_()
    {
...
private:
    ...
    //
    Node *createFuncNode(const Type& ty, const std::string& funcName, std::vector<Node *> params,
                         Node *body);
     ...
     * \ Generate Quadruples.
     */
    inline void gen_quad(const std::string& q1, const std::string& q2 = std::string(),
                         const std::string& q3 = std::string(), const std::string& q4 = std::string())
    {
...
inline bool isNumber(const std::string& str)
{
    ...
    if (!((str.at(0) >= '0' && str.at(0) <= '9') || (str.at(0) == '-' || str.at(0) == '+'))) return false;

    bool _has_dot = false;
    for (size_t i = 1; i < str.size(); ++i) {
        if (!((str.at(i) >= '0' && str.at(i) <= '9') || (str.at(i) == '.' && _has_dot == false)))
            return false;
        if (str.at(i) == '.') _has_dot = true;
    }
    return true;
}
