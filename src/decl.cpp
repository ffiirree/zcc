...
void Parser::declaration(std::vector<Node *>& list, bool isGlo)
{
    ...
    for (;;) {
        ...
        if (sclass == K_TYPEDEF) {
            ...
            log_e(ts_.location(), "Do not support static.");
        }
        ...
        if (!next_is(',')) log_e(ts_.location(), "';' or ',' are expected, but got not");
    }
}
...
Node *Parser::designator_list()
{
    if (next_is('[')) {
        ...
        Node *r = nullptr;
        Token t = ts_.next();
        if (t.getId() == T_IDENTIFIER) *r = localenv->search(t.getSval());
        if (r->getKind() == 0) log_e(ts_.location(), "init list error");
        return r;
    }
...
| '(' param_type_list ')' direct_declarator_tail                                  # Function declarators
(including prototypes) | '(' [ID_list] ')' direct_declarator_tail                                        #
Function declarators (including prototypes) | empty
*/
...
{
    if (is_type(ts_.peek())) return decl_specifiers(sclass);

    log_w(ts_.location(), "missing type, defalut 'int'.");
...
Type Parser::direct_decl_tail(Type *retty, std::vector<Node *>& params, int decl_type)
{
    ...
    if (next_is('(')) {
        if (decl_type == DECL_BODY) decl_type = NODE_FUNC_DECL;
...
Type Parser::decl_specifiers(int *rsclass)
{
    ...
    for (;;) {
        t = ts_.next();
        if (t.getType() == T_EOF) log_e(ts_.location(), "end of file.");

        if (kind == 0 && t.getType() == T_IDENTIFIER && !custom_type.type) {
            custom_type = getCustomType(t.getSval());
            ts_.next();
            if (custom_type.type == 0) log_e(ts_.location(), "Undefined type: " + t.getSval());
        }
        ...
        switch (t.getId()) {
            // Type specifiers
#define type_spec_cheak(cheak, val, _t)                                                                    \
    do {                                                                                                   \
        if (cheak)                                                                                         \
            log_e(ts_.location(), "error " + std::string(_t) + " specifier");                              \
        else                                                                                               \
            cheak = val;                                                                                   \
    } while (0)
        ...
        case K_STRUCT:
            if (custom_type.type != 0) log_e(ts_.location(), "error struct specifier.");
            custom_type = struct_def();
            break;
        case K_UNION: log_e(ts_.location(), "no union."); break;
#undef type_spec_cheak

        case K_CONST: log_e(ts_.location(), "no const."); break;
        case K_RESTRICT: log_e(ts_.location(), "no restrict."); break;
        case K_VOLATILE:
            log_e(ts_.location(), "no volatile.");
            break;

            // Function specifiers
        case K_INLINE: log_e(ts_.location(), "no inline."); break;
        case ELLIPSIS: type.create(ELLIPSIS, 0, false); break;

        default: goto _end; break;
        }
    }
_end:
    ts_.back();

    if (current_class) *rsclass = current_class;

    if (custom_type.type != 0) return custom_type;
...
{
    if (ts_.peek().getType() == T_IDENTIFIER) ts_.next();
    ...
    do {
        ...
        if (t.getType() == T_IDENTIFIER) {
            ...
            if (!n.name().empty()) log_e(ts_.location(), "redefined var : " + _n);
            if (!searchEnum(_n).empty()) log_e(ts_.location(), "redefined var :" + _n);

            if (next_is('=')) {
                t = ts_.next();

                if (t.getType() == T_INTEGER) {
                    if (atoi(t.getSval().c_str()) < _val) log_e(ts_.location(), "Inc val.");
                    _val = atoi(t.getSval().c_str()) + 1;
                    _v   = t.getSval();
                }
                else {
                    log_e(ts_.location(), "need integer.");
                }
            }
            else {
            ...
            log_e(ts_.location(), "Ench member need a name.");
...
{
    if (enum_const.empty()) return std::string();
    std::map<std::string, std::string>::iterator iter = enum_const.find(key);
    if (iter != enum_const.end()) return iter->second;
    return std::string();
}
...
Type Parser::struct_def()
{
    ...
    Token t = ts_.peek();
    if (t.getType() != T_IDENTIFIER) {
        log_e(ts_.location(), "Struct need a type name.");
    }
    t              = ts_.next();
    ...
    while (ts_.peek().getId() != '}' && ts_.peek().getType() != T_EOF) {
        ...
        if (ts_.peek().getType() != T_IDENTIFIER) log_e(ts_.location(), "expect a identier.");
