...
void Env::push_back(Node& n)
{
    if (n.kind_ == NODE_FUNC) {
        ...
        if (r.kind_ == NODE_FUNC_DECL) {
            ...
            log_e("Function redefined: " + n.name());
        }
    }
...
{
    for (size_t i = 0; i < labels.size(); ++i) {
        if (!enLabels.at(i)) log_e("Label '" + labels.at(i) + "' is undefined");
    }
    return true;
}
...
Node *Parser::funcDef()
{
    ...
    if (functype.type == PTR) {
        log_e(ts_.location(), "Ptr not can be function.");
    }
...
Node *Parser::param_decl(int decl_type)
{
    ...
    if (decl_type == NODE_FUNC_DECL) return createFuncDeclParams(type);
...
Node *Parser::func_body(Type& functype, std::string name, std::vector<Node *>& params)
{
    ...
    if (noReturnValue) {
        if (functype.retType->type == K_VOID)
            ...
            log_e(ts_.location(), "Function need a return statement");
    }
...
{
    if (t.getType() != T_KEYWORD) return 0;
    switch (t.getId()) {
...
{
    if (ts_.next().getId() == id) return true;
    ts_.back();
    return false;
}
...
Node *Parser::createFuncNode(const Type& ty, const std::string& funcName, std::vector<Node *> params,
                             Node *body)
{
...
Node *Parser::createGLoVarNode(const Type& ty, const std::string& name)
{
    ...
    if (cheak_redefined(globalenv, r->name()))
        log_e(ts_.location(), "redefined global variable : " + r->name());
...
Node *Parser::createLocVarNode(const Type& ty, const std::string& name)
{
    ...
    if (cheak_redefined(localenv, r->name()))
        log_e(ts_.location(), "redefined local variable : " + r->name());
...
{
    if (t.getType() == T_IDENTIFIER) return getCustomType(t.getSval()).type != 0;

    if (t.getType() != T_KEYWORD) return false;

    switch (t.getId()) {
#define keyword(id, _, is)                                                                                 \
    case id: return is;
        KEYWORD_MAP
#undef keyword
...
bool Parser::is_keyword(const Token& t, int id) { return (t.getType() == T_KEYWORD && t.getId() == id); }
...
void Parser::skip_parenthesis(int *count)
{
    for (;;) {
        ...
        if (t.getType() == T_EOF) log_e(ts_.location(), "error eof");

        if (is_keyword(t, '(')) skip_parenthesis(count);

        if (is_keyword(t, ')')) break;
    }
}
void Parser::expect(int id)
{
    ...
    if (t.getId() != id)
        log_e(ts_.location(),
              std::string("expect '") + (char)id + "', but not is '" + (char)t.getId() + "'");
}
...
bool Parser::is_arithtype(const Type& ty) { return is_inttype(ty) || is_floattype(ty); }

void Parser::createQuadFile()
{
    out.open(_of_name, std::ios::out | std::ios::binary);
    if (!out.is_open()) log_e("Create file filed!");
}
...
void Parser::pushQuadruple(const std::string& name) { quad_arg_stk_.push_back(name); }
...
void Parser::createUnaryQuadruple(const std::string& op)
{
    ...
    if (op == "~" && isNumber(quad_arg_stk_.back())) {
        int _n = atoi(quad_arg_stk_.back().c_str());
        quad_arg_stk_.pop_back();
        _n = ~_n;
...
{
    if (quad_arg_stk_.size() < 2) log_e("Compute " + op + " operand < 2.");

    std::string var1 = quad_arg_stk_.back();
    quad_arg_stk_.pop_back();
    std::string var2 = quad_arg_stk_.back();
    quad_arg_stk_.pop_back();
...
void Parser::createRelOpQuad(const std::string& op)
{
    ...
    std::string E1 = quad_arg_stk_.back();
    quad_arg_stk_.pop_back();
    std::string E2 = quad_arg_stk_.back();
    quad_arg_stk_.pop_back();
...
void Parser::createQuadruple(const std::string& op)
{
    ...
    if (op == "=") {
        std::string v1 = quad_arg_stk_.back();
        quad_arg_stk_.pop_back();
        std::string v2 = quad_arg_stk_.back();
        quad_arg_stk_.pop_back();
        _GENQ3_(op, v1, v2);
    }
        ...
        std::string v2 = quad_arg_stk_.back();
        quad_arg_stk_.pop_back();
        std::string v3 = quad_arg_stk_.back();
        quad_arg_stk_.pop_back();
...
void Parser::createFuncQuad(std::vector<Node *>& params)
{
    ...
    if ((fn.kind_ != NODE_FUNC && fn.kind_ != NODE_FUNC_DECL) || (fn.params.size() != params.size()))
        log_e(ts_.location(), "func call parms size error.");
...
_skip_cheak_params_num:
    ...
    if (maxCallSize_ < callSize) maxCallSize_ = callSize;
}

void Parser::createIncDec()
{
    for (;;) {
        if (incDecStk_.empty()) return;
...
std::string Parser::getOverLoadName(const std::string& name, std::vector<Node *>& _p)
{
    ...
    for (size_t i = 0; i < _p.size(); ++i) {
        ...
        case K_STRUCT:
        case K_TYPEDEF: log_e(ts_.getPos(), "Unspport struct and typedef overload."); break;
        default: log_e(ts_.getPos(), "Unspport type overload."); break;
        }
    }
    return _name_r;
}
...
std::vector<int> *Parser::merge(std::vector<int> *p1, std::vector<int> *p2)
{
    std::vector<int> *list = new std::vector<int>();
...
void Parser::out_quad()
{
    for (const auto& q : quadStk_) {
        ...
        out << "\t" << std::left << std::setw(7) << std::get<0>(q) << " " << std::left << std::setw(10)
            << std::get<1>(q) << " " << std::left << std::setw(10) << std::get<2>(q) << " "
...
#define _L(S) std::left << std::setw(S)
    os << _L(15) << "[name]" << _L(15) << "[kind]" << _L(20) << "[type]" << _L(10) << "[size]" << _L(15)
       << "[offset]" << std::endl;
    ...
    for (size_t i = 0; i < env.size(); ++i) {
        ...
        default: log_e("unknown Node kind"); break;
        }
        os << _L(20);
        switch (n.type_.type) {
        ...
        case NODE_FUNC:
            ...
            break;
        default: log_e("unknown Node type"); break;
        }
...
void Parser::printSymbolTable()
{
    ...
    if (!stf.is_open()) log_e("Open " + stfname + "failed.");
