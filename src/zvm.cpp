...
VirtualMachine::VirtualMachine(bool use, bool debug)
    : use_(use),
      debug_(debug)
{
    // stack
    ebp       = (intptr_t)(stk_ + 24);
    esp       = ebp;
    heap_ptr_ = (intptr_t)heap_;
...
{
    if (ai.operator_ == call) return;
    if (ai.operator_ == leave) return;
    if (ai.operator_ == ret) return;
    if (ai.operator_ == exitvm) return;
    if (ai.operator_ >= jg && ai.operator_ <= jne) return;

    ai.loperand_ = getOperandAddr(ai.lstr_);
    if (!ai.rstr_.empty()) ai.roperand_ = getOperandAddr(ai.rstr_);
}
...
_begin_debug_:
            ...
            if (c1 == "n") {
                ...
                std::cout << "eax=" << std::left << std::setw(10) << eax << " ebx=" << std::left
                          << std::setw(10) << ebx << " ecx=" << std::left << std::setw(10) << ecx
                          << " edx=" << std::left << std::setw(10) << edx << std::endl;
                std::cout << "ebp=" << std::left << std::setw(10) << ebp << " esp=" << std::left
                          << std::setw(10) << esp << std::endl
                          << std::endl;
            }
            else if (c1 == "q") {
                return;
...
Instruction VirtualMachine::getInsByOp(const std::string& name)
{
    for (const auto& ins : instructions_) {
        if (name == ins.first) return ins.second;
    }
    log_e("unknown operator:" + name);
    return INS_NULL;
}
...
bool VirtualMachine::isImmediate(const std::string& name)
{
    for (size_t i = 0; i < name.size(); ++i) {
        if (name.at(i) != '$' && (name.at(i) > '9' || name.at(i) < '0')) return false;
    }
    return true;
}
...
void *VirtualMachine::getOperandAddr(const std::string& name)
{
    ...
    switch (name.at(0)) {
    case '$':
        if (isImmediate(name)) return new int(getImmediate(name));
    case '_':
        iter = data_.find(name);
        if (iter != data_.end())
            return (int *)iter->second;
        else
            log_e(name + " is not exist.");
        break;
...
case_0_9:
        ...
        if (_off.empty()) _off.push_back('0');

        for (; name.at(i) != ')'; ++i) {
            switch (name.at(i)) {
case_a_z:
            case '%': _reg.push_back(name.at(i));
            }
        }
